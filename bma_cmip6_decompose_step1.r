...
results <- list()
setwd("~/Desktop/BMA")
load("all_df_allmodel_new.RData") # cum co2, with yearly temp
...
alpha_post_mean54 = alpha_post_mean
beta_post_mean54 = beta_post_mean
cat ("54 CIMP6 models \n")
cat ("Posterior mean [alpha, beta] is [", alpha_post_mean54,", ",beta_post_mean54, "] \n")

cov_matrix54 <- matrix(c(alpha_post_var, alpha_beta_post_cov,
...
samples <- mvrnorm(n = 1000, mu = c(alpha_post_mean, beta_post_mean), Sigma = cov_matrix54)
...
x_t <- x_t/1000 # /1000 means changing from Gt to 1000Gt
...
co2_matrix <- sapply(1:1000, function(i) proj.evals.2024.ar1.const$trajs.annual.worldwide[[i]]$CO2/ 1e12) # row is year, col is traj =1000, 1000Gt
...
for (i in 1:1000) {
  parameters <- compute_distr_deltaT(
    delta_x = delta_x, 
    t1 = 2024, 
    phi_post = phi_post, 
  ...
  parameters$temp_change <- rnorm(nrow(parameters), parameters$mean, sqrt(parameters$var))
