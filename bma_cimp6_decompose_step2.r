...
estimate_model <- function(df) {
  ...
  alpha_var <- (1 / (1 - phi_k))^2 * vcov_matrix_prime[1, 1] 
  beta_var <- vcov_matrix_prime[2, 2]  
...
compute_distr_deltaT2 <- function(delta_x, t1, phi_post_mean, sigma_squared_post_mean, beta_post_mean, cov_matrix) {
  # Initialize the variance vector
  m <- delta_x$Year - t1
  var_part1 <- numeric(length(m))
  ...
  for (i in 1:length(m)) {     
    mi <- m[i]     
    phi_j <- phi_post_mean
    sigma_j2 <- sigma_squared_post_mean
    
    term1 <- sigma_j2 * (phi_j^mi - 1)^2
    term2 <- (1 - phi_j^(2 * (t1-1850))) / (1 - phi_j^2)
    term3 <- sigma_j2 * (1 - phi_j^(2 * mi)) / (1 - phi_j^2)
    
    var_part1[i] <- term1 * term2 + term3
  }
...
############# none step2 #############
######################################
load("~/Desktop/BMA/proj_evals_ar1const_2024.rda")
# 51 CMIP6
...
phi_post_mean <- coef(phi_model)[2]
# Compute residuals of the AR(1) model
epsilon_t <- residuals(phi_model)

# # Estimate sigma^2
# sigma_squared_post[i] <- var(epsilon_t)
sigma_squared_post_mean <- var(epsilon_t)
# }



###
# get median of CO2 first
num_years <- length(proj.evals.2024.ar1.const$trajs.annual.worldwide[[1]]$Year)

co2_matrix <- sapply(1:1000, function(i) proj.evals.2024.ar1.const$trajs.annual.worldwide[[i]]$CO2/ 1e12) # row is year, col is traj =1000
co2_medians <- apply(co2_matrix, 1, median) 


distr_deltaT_none <- list(
  Year = proj.evals.2024.ar1.const$trajs.annual.worldwide[[1]]$Year[-1],  # 2025:2100
  parameters = vector("list", 1000),
  temp.change.traj = vector("list", 1000)
)


cumCO2 <- cumsum(co2_medians)

delta_x <- data.frame(
  Year = proj.evals.2024.ar1.const$trajs.annual.worldwide[[1]]$Year[-1],  # 2025:2100
  deltaCO2 = cumCO2[-1] - cumCO2[1] 
)

parameters <- compute_distr_deltaT2(
  delta_x = delta_x, 
  t1 = 2024, 
  phi_post_mean = phi_post_mean, 
  sigma_squared_post_mean = sigma_squared_post_mean, 
  beta_post_mean = beta_post_mean, 
  cov_matrix = cov_matrix
)

temp_change_matrix <- sapply(1:1000, function(i) {
  rnorm(nrow(parameters), parameters$mean, sqrt(parameters$var))
})


distr_deltaT_none$temp.change.traj <- split(temp_change_matrix, col(temp_change_matrix))
distr_deltaT_none$parameters <- replicate(1000, parameters[, c("Year", "mean", "var")], simplify = FALSE)
...
load("~/Desktop/BMA/summary_anomaly.rda")
...
load("~/Desktop/BMA/combined.rda")
load("~/Desktop/BMA/summary_proj.rda")
...
############# adjust step2 #############
######################################
load("~/Desktop/BMA/proj_evals_2024_adjusted.Rda")
# 51 CMIP6
### obtain posterior sample
x_t <- cumsum(data.carbon1$World[57:66])* 3.664/1000 # 57:66 means 2015-2024, /1000 means changing from Mt to Gt
...
num_years <- length(proj.evals.2024.adjusted$trajs.annual.worldwide[[1]]$Year)

co2_matrix <- sapply(1:1000, function(i) proj.evals.2024.adjusted$trajs.annual.worldwide[[i]]$CO2/ 1e12 ) # row is year, col is traj =1000
co2_medians <- apply(co2_matrix, 1, median)   


distr_deltaT_adj <- list(
  Year = proj.evals.2024.adjusted$trajs.annual.worldwide[[1]]$Year[-1],  # 2025:2100
  parameters = vector("list", 1000),
  temp.change.traj = vector("list", 1000)
)
...
distr_deltaT_adj$temp.change.traj <- split(temp_change_matrix, col(temp_change_matrix))

distr_deltaT_adj$parameters <- replicate(1000, parameters[, c("Year", "mean", "var")], simplify = FALSE)
...
num_years <- length(proj.evals.2024.adjusted$trajs.annual.worldwide.cont[[1]]$Year)


co2_matrix <- sapply(1:1000, function(i) proj.evals.2024.adjusted$trajs.annual.worldwide.cont[[i]]$CO2/ 1e12 ) # row is year, col is traj =1000
co2_medians <- apply(co2_matrix, 1, median)  

distr_deltaT_cont <- list(
  Year = proj.evals.2024.adjusted$trajs.annual.worldwide.cont[[1]]$Year[-1],  # 2025:2100
  parameters = vector("list", 1000),
  temp.change.traj = vector("list", 1000)
)
...
distr_deltaT_cont$temp.change.traj <- split(temp_change_matrix, col(temp_change_matrix))

distr_deltaT_cont$parameters <- replicate(1000, parameters[, c("Year", "mean", "var")], simplify = FALSE)
...
num_years <- length(proj.evals.2024.adjusted$trajs.annual.worldwide.usa[[1]]$Year)

co2_matrix <- sapply(1:1000, function(i) proj.evals.2024.adjusted$trajs.annual.worldwide.usa[[i]]$CO2/ 1e12 ) # row is year, col is traj =1000
co2_medians <- apply(co2_matrix, 1, median)   

distr_deltaT_usa <- list(
  Year = proj.evals.2024.adjusted$trajs.annual.worldwide.usa[[1]]$Year[-1],  # 2025:2100
  parameters = vector("list", 1000),
  temp.change.traj = vector("list", 1000)
)
...
distr_deltaT_usa$temp.change.traj <- split(temp_change_matrix, col(temp_change_matrix))

distr_deltaT_usa$parameters <- replicate(1000, parameters[, c("Year", "mean", "var")], simplify = FALSE)
